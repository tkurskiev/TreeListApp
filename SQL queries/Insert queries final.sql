...
--INNER JOIN aggregatesinsertqte qte ON ca.name = qte.name
...
--INNER JOIN app.catalog_level as cl ON cl.name = ca.name
	--AND cl.parent_id = (SELECT id FROM app.catalog_level WHERE name = c.name)
